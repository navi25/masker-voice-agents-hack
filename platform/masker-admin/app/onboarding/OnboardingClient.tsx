  ...
  async function createWorkspace() {
    ...
    try {
      const res = await fetch("/api/onboarding/org", {
        ...
        body: JSON.stringify({ name: orgName, slug: slugify(orgName), framework }),
      });
  ...
  async function createApiKey() {
    ...
    try {
      const res = await fetch("/api/keys", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ label: keyLabel, permissions, environment: "production", orgId }),
      });
      if (!res.ok) throw new Error((await res.json()).error ?? "Failed");
      const data = await res.json() as { prefix: string; full_key: string };
      setCreatedKey({ prefix: data.prefix, fullKey: data.full_key });
      setStep(4);
    } catch (e: unknown) {
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] p-8">
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {orgName && <p className="text-[11px] text-[#9ca3af] mt-1 font-mono">slug: {slugify(orgName)}</p>}
                </div>
                <div>
                  <label className="block text-[12px] font-medium text-[#374151] mb-2">Compliance framework</label>
                  <div className="grid grid-cols-2 gap-2">
                    {FRAMEWORKS.map((f) => (
                      <button key={f.value} onClick={() => setFramework(f.value)}
                        className={cn("text-left p-3 rounded-lg border text-[13px] transition-colors",
                          framework === f.value ? "border-indigo-500 bg-indigo-50" : "border-[#e5e7eb] hover:border-[#d1d5db]")}>
                        <div className={cn("font-semibold mb-0.5", framework === f.value ? "text-indigo-700" : "text-[#0d0f12]")}>{f.label}</div>
              ...
              <button onClick={createWorkspace} disabled={!orgName.trim() || loading}
          ...
          {step === 2 && (
            <div>
              ...
              <p className="text-[13px] text-[#6b7280] mb-6">Masker uses envelope encryption for all session data.</p>
              <div className="space-y-3">
                <button onClick={() => setKmsProvider("masker")}
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <span className={cn("text-[14px] font-semibold", kmsProvider === "masker" ? "text-indigo-700" : "text-[#0d0f12]")}>Masker-managed KMS</span>
                    {kmsProvider === "masker" && <Check className="w-4 h-4 text-indigo-600" />}
                  </div>
                  <p className="text-[12px] text-[#6b7280]">Auto-provisioned, auto-rotated. Zero ops overhead.</p>
                ...
                <div className="relative">
                  <div className="w-full text-left p-4 rounded-lg border border-[#e5e7eb] opacity-50 cursor-not-allowed select-none">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[14px] font-semibold text-[#0d0f12] flex items-center gap-2"><Lock className="w-3.5 h-3.5" /> Bring Your Own KMS</span>
                      <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-[#f3f4f6] text-[#9ca3af]">Enterprise</span>
                    </div>
                    <p className="text-[12px] text-[#9ca3af]">Connect AWS KMS, GCP Cloud KMS, or HashiCorp Vault.</p>
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-[12px] font-medium text-[#6b7280] bg-white px-3 py-1 rounded-full border border-[#e5e7eb] shadow-sm">Contact us to unlock</span>
                  </div>
                </div>
              </div>
              {error && <p className="mt-4 text-[13px] text-red-600">{error}</p>}
              <button onClick={setupKms} disabled={loading}
          ...
          {step === 3 && (
            <div>
              ...
              <p className="text-[13px] text-[#6b7280] mb-6">Authenticates your SDK. Create more keys with different scopes later.</p>
              <div className="space-y-4">
                <div>
                  <label className="block text-[12px] font-medium text-[#374151] mb-1.5">Key label</label>
                  <input type="text" value={keyLabel} onChange={(e) => setKeyLabel(e.target.value)} placeholder="Production"
                    className="w-full px-3 py-2 text-[14px] border border-[#e5e7eb] rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
              ...
              {error && <p className="mt-4 text-[13px] text-red-600">{error}</p>}
              <button onClick={createApiKey} disabled={!keyLabel.trim() || permissions.length === 0 || loading}
                className="mt-6 w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#0d0f12] text-white text-[14px] font-medium rounded-lg hover:bg-[#1f2937] transition-colors disabled:opacity-40">
          ...
          {step === 4 && createdKey && (
            <div>
              ...
              <p className="text-[13px] text-[#6b7280] mb-6">Copy your key now — it won&apos;t be shown again.</p>
              <div className="bg-[#0d0f12] rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button onClick={() => navigator.clipboard.writeText(createdKey.fullKey)}
                    className="text-[11px] text-[#9ca3af] hover:text-white transition-colors">Copy</button>
                </div>
                <code className="text-[13px] text-green-400 font-mono break-all">{createdKey.fullKey}</code>
              </div>
              <div className="space-y-3 mb-6">
                <a href={`/api/sdk/download?format=python&key=${encodeURIComponent(createdKey.fullKey)}&framework=${framework}&org=${encodeURIComponent(orgName)}`}
                ...
                <a href={`/api/sdk/download?format=json&key=${encodeURIComponent(createdKey.fullKey)}&framework=${framework}&org=${encodeURIComponent(orgName)}`}
                  className="flex items-center justify-between p-4 rounded-lg border border-[#e5e7eb] hover:border-indigo-300 hover:bg-indigo-50 transition-colors group">
                  <div className="flex items-center gap-3">
                    <span className="text-xl">⚙️</span>
                    <div>
                      <div className="text-[13px] font-semibold text-[#0d0f12] group-hover:text-indigo-700">JSON Config</div>
                      <div className="text-[11px] text-[#9ca3af]">masker_config.json — for any language or framework</div>
                    </div>
                  </div>
                  <Download className="w-4 h-4 text-[#9ca3af] group-hover:text-indigo-600" />
                </a>
              ...
              <button onClick={() => router.push("/overview")}
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-[14px] font-medium rounded-lg hover:bg-indigo-700 transition-colors">
