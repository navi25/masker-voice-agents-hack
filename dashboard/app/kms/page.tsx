...
import { KMS_KEYS, type KmsKey } from "@/lib/mock-data";
...
export default function KmsPage() {
  const [keys, setKeys] = useState<KmsKey[]>(KMS_KEYS);
  ...
  function rotate(id: string) {
    setKeys((ks) =>
      ks.map((k) =>
        k.id === id
          ? { ...k, status: "rotating" as const, lastRotated: new Date().toISOString().slice(0, 10) }
          : k
      )
    );
  }
  ...
  function disable(id: string) {
    setKeys((ks) =>
      ks.map((k) => (k.id === id ? { ...k, status: "disabled" as const } : k))
    );
  }
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      disabled={k.status === "disabled" || k.status === "rotating"}
                      onClick={() => rotate(k.id)}
